...
class Goal {
  ...
  Goal(this.name, this.targetAmount, this.finishDate, this.currentAmount, this.priorityLabel);

  Goal.withKey(this.key, this.name, this.targetAmount, this.finishDate, this.currentAmount, this.priorityLabel);
}
...
class _FinancialGoalsPageState extends State<FinancialGoalsPage> {
  ...
  Future<int> calculateCurrentSavings(Goal goal) async {
    ...
    if (user != null) {
      ...
      final DatabaseReference goalRef = databaseReference
          .child('goals')
          .child(user.uid)
          .child(goal.key!);
  ...
  Future<String> getSavingsSuggestions() async {
    ...
    try {
      final DatabaseEvent event = await expensesRef.child('cash').child(user.uid).once();
      final DataSnapshot cashExpensesSnapshot = event.snapshot;
      final DatabaseEvent event1 = await expensesRef.child('online').child(user.uid).once();
      final DataSnapshot onlineExpensesSnapshot = event1.snapshot;

      final cashExpensesData = cashExpensesSnapshot.value as Map<dynamic, dynamic>? ?? {};
      final onlineExpensesData = onlineExpensesSnapshot.value as Map<dynamic, dynamic>? ?? {};


      final lastMonthCashExpenses = filterExpensesForLastMonth(cashExpensesData, lastMonthStart, lastMonthEnd);
      final lastMonthOnlineExpenses = filterExpensesForLastMonth(onlineExpensesData, lastMonthStart, lastMonthEnd);
      ...
      final thresholdValues = {
        ...
        'Food and Dining': 2000, // Suggested threshold value for Food and Dining
        'Utilities': 1000, // Suggested threshold value for Utilities
        'Personal Expense': 2000, // Suggested threshold value for Personal Expense
      ...
      final cashCategoryWiseExpenses = categorizeExpenses(lastMonthCashExpenses);
      final onlineCategoryWiseExpenses = categorizeExpenses(lastMonthOnlineExpenses);

      for (final category in cashCategoryWiseExpenses.keys) {
        print("enter in");
        final totalExpense = calculateTotalExpense(cashCategoryWiseExpenses[category]!);
        print("enter in2");
        ...
        if (totalExpense > threshold) {
          ...
          suggestions.add('Reduce spending on $category (Cash). You can save \₹$savings.');
        }
        else{
          print("enter in else");
        }
      ...
      for (final category in onlineCategoryWiseExpenses.keys) {
        print("enter in ");
        final totalExpense = calculateTotalExpense(onlineCategoryWiseExpenses[category]!);
        final threshold = thresholdValues[category] ?? 0.0;

        if (totalExpense > threshold) {
          print("enter in if");
          final savings = totalExpense - threshold;
          suggestions.add('Reduce spending on $category (Online). You can save \₹${savings.toStringAsFixed(2)}.');
        }
      }

      if (suggestions.isEmpty) {
        suggestions.add('Great job! Your expenses are within budget for all categories.');
      }
  ...
  Map<String, dynamic> filterExpensesForLastMonth(Map<dynamic, dynamic> expensesData, DateTime start, DateTime end) {
    ...
    for (final entry in expensesData.entries) {
      ...
      if (expenseDate != null && expenseDate.isAfter(start) && expenseDate.isBefore(end)) {
  ...
  Map<String, List<Map<dynamic, dynamic>>> categorizeExpenses(Map<dynamic, dynamic> expensesData) {
  ...
  void _editGoal(Goal goal) {
    ...
    currentAmountController.text = goal.currentAmount.toString(); // Populate current amount
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        ...
        TextField(
        controller: currentAmountController, // Include the current amount field
        decoration: InputDecoration(
    ...
    if (nameController.text.isNotEmpty &&
      ...
      if (user != null) {
        ...
        await goalRef.update({
          ...
          'currentAmount': double.parse(currentAmountController.text), // Include currentAmount
        });
  ...
  void _addGoal() async {
    if (nameController.text.isNotEmpty &&
        targetAmountController.text.isNotEmpty &&
        selectedDate != null &&
        currentAmountController.text.isNotEmpty) { // Check if the current amount is not empty
      final newGoal = Goal(
        nameController.text,
      ...
      if (user != null) {
        ...
        final DatabaseReference newGoalRef = databaseReference
            .child('goals')
            .child(user.uid)
            .push();
  ...
  Future<List<Goal>> fetchUserGoals() async {
    ...
    if (user != null) {
      ...
      print('User is not authenticated'); // Add this line to check authentication
    }
            ...
            isLoading // Show a loading indicator if data is being fetched
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final remainingDays = goal.finishDate.difference(DateTime.now()).inDays;
                  return Dismissible(
                    key: Key(goal.name),
                              ...
                              builder: (BuildContext context, AsyncSnapshot<double> snapshot) {
                                if (snapshot.connectionState == ConnectionState.waiting) {
                                  return LinearProgressIndicator(
                                    value: 0.0, // While loading, show 0% progress
          ...
          content: SingleChildScrollView(
            child: Column(
              children: [
                ...
                TextField(
                  controller: currentAmountController, // Add Current Amount field
                  decoration: InputDecoration(
                    labelText: 'Current Amount (\₹)',
                    labelStyle: TextStyle(color: Color(0xff3AA6B9)),
                  ),
